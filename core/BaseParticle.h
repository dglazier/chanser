...
namespace chanzer{
  ...
  public:
    ...
    HSPosition Vertex() const {return _Vertex;}
  ...
  inline void chanzer::BaseParticle::SetCLAS12Particle(clas12::region_particle *p){
    _C12Particle=p;
    auto pbank=p->par();
    SetPDGcode(pbank->getPid());
    _Charge=pbank->getCharge();
    SetXYZT(pbank->getPx(),pbank->getPy(),pbank->getPz(),pbank->getP());
    SetVertex(pbank->getVx(),pbank->getVy(),pbank->getVz());
    _Time=p->getTime();
    _Path=p->getPath();
