...
namespace chanser{
  ...
  void FSSkeleton::DeclareDetected(){
    ...
    for(Int_t io=0;io<_finals->GetEntries();io++){
      ...
      ContinueLineAfter(Form("    BaseParticle   _%s = BaseParticle{\"%s\"};",TString(sparticle(0,sparticle.First(":"))).Data(),TString(sparticle(sparticle.First(":")+1,sparticle.Sizeof())).Data()));
      }
    }
